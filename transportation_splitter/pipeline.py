...
class OvertureTransportationSplitter:
    ...
    def split(self, filter_wkt: str | None = None) -> DataFrame:
        """
        Run the splitting pipeline.

        Args:
            filter_wkt: Optional WKT polygon to spatially filter input data.
                        Only features intersecting this polygon will be processed.

        Returns:
            DataFrame containing split segments and connectors
        """
        logger.info(f"Starting split with filter_wkt={filter_wkt}")
        logger.info(f"wrangler: {self.wrangler}")
        logger.info(f"config: {self.cfg}")

        # Track if upstream steps recomputed - if so, downstream caches are stale
        # This ensures that if we apply a new filter, all downstream steps recompute
        upstream_recomputed = False

        # Step 1: Read and optionally filter
        filtered_df, recomputed = self._read_and_filter(filter_wkt)
        upstream_recomputed = upstream_recomputed or recomputed
        ...
        joined_df, recomputed = self._join_and_prepare(filtered_df, upstream_recomputed)
        upstream_recomputed = upstream_recomputed or recomputed

        # Step 3: Split segments
        split_df, recomputed = self._split_segments(joined_df, lr_columns, upstream_recomputed)
        upstream_recomputed = upstream_recomputed or recomputed

        # Step 4: Format output
        final_df = self._format_output(split_df, filtered_df, upstream_recomputed)

        return final_df

    def _read_and_filter(self, filter_wkt: str | None) -> tuple[DataFrame, bool]:
        """Read input data and optionally apply spatial filter.

        Returns:
            Tuple of (DataFrame, recomputed) where recomputed is True if data was
            freshly computed (not loaded from cache).
        """
        if filter_wkt is None:
            return self.wrangler.read(self.spark, SplitterStep.read_input), False

        # Check if we can reuse existing intermediate
        can_reuse = (
            self.cfg.write_intermediate_files
            and self.wrangler.check_exists(self.spark, SplitterStep.spatial_filter)
            and self.cfg.reuse_existing_intermediate_outputs
        )

        if can_reuse:
            return self.wrangler.read(self.spark, SplitterStep.spatial_filter), False
        ...
        if self.cfg.write_intermediate_files:
            self.wrangler.write(filtered_df, SplitterStep.spatial_filter)

        return filtered_df, True

    def _join_and_prepare(self, filtered_df: DataFrame, upstream_recomputed: bool) -> tuple[DataFrame, bool]:
        """Join segments with connectors and pre-compute segment length.

        Args:
            filtered_df: The filtered input DataFrame
            upstream_recomputed: If True, upstream data changed so we must recompute
        ...
        # Check if we can reuse existing intermediate
        can_reuse = (
            not upstream_recomputed
            and self.cfg.write_intermediate_files
    ...
    def _split_segments(
        self, joined_df: DataFrame, lr_columns: list[str], upstream_recomputed: bool
        ...
        if can_reuse:
            return self.wrangler.read(self.spark, SplitterStep.raw_split), False
    ...
    def _format_output(self, split_df: DataFrame, filtered_df: DataFrame, upstream_recomputed: bool) -> DataFrame:
        ...
        can_reuse = (
            not upstream_recomputed
            and self.cfg.write_intermediate_files
            and self.wrangler.check_exists(self.spark, SplitterStep.segment_splits_exploded)
            and self.cfg.reuse_existing_intermediate_outputs
        )

        if can_reuse:
            final_segments_df = self.wrangler.read(self.spark, SplitterStep.segment_splits_exploded)
        else:
            ...
            if self.cfg.write_intermediate_files:
                self.wrangler.write(flat_splits_df, SplitterStep.segment_splits_exploded)
                final_segments_df = self.wrangler.read(self.spark, SplitterStep.segment_splits_exploded)
            else:
        ...
        if not self.cfg.skip_debug_output:
            type_counts = loaded_final_df.groupBy("type").agg(F.count("*").alias("count")).collect()
